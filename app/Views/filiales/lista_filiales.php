    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">Listado de Oficinas</li>
        </ol>
      </div>
    </div>
  </div><!-- /.container-fluid -->
...
<section class="content">
  ...
  <section class="content">
    ...
    <div>
        <?= anchor('oficinas/listarOficinas', 'Lista', ['class' => 'btn btn-primary']); ?>
    </div>
    ...
    <tr>
        <td><?= $oficina->id; ?></td>
        <td><?= esc($oficina->nombre_filial); ?></td>
        <td>
            <?php if ($oficina->nombre && $oficina->apellido): ?>
                <?= esc($oficina->nombre . ' ' . $oficina->apellido); ?>
            <?php else: ?>
                Sin usuario asignado
            <?php endif; ?>
        </td>
        <td><?= esc($oficina->fecha_creacion); ?></td>


        <td>
        <a href="<?php echo 'formulario_editar_oficina/' . $oficina->id ?>" class="btn btn-success" type="button">Editar Oficina</a>
        </td>
