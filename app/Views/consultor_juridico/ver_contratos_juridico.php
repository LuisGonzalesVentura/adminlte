...
<section class="content">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Contratos Jurídicos</h3>
                    </div>
                    <div class="card-body">
                        <table class="table">
                            <thead>
    ...
    <?php if (!empty($contratosConsultores)): ?>
        <?php foreach ($contratosConsultores as $contrato): ?>
            <tr>
                <td>
                    <a href="<?= base_url('pdfscontratos/' . $contrato->archivo_c_j) ?>" target="_blank"><?= $contrato->archivo_c_j ?></a>
                </td>
                <td>
                    <?php
                    // Obtener el nombre del tipo de contrato
                    $tipoContratoModel = new TipoContrato();
                    $tipoContrato = $tipoContratoModel->find($contrato->tipo_contrato_c_j);
                ...
                <td><?= $contrato->fecha_inicio_contrato_c_j ?></td>
                <td><?= $contrato->fecha_finalizacion_contrato_c_j ?></td>
                <!-- Agregar más columnas si es necesario -->
            </tr>
        <?php endforeach; ?>
    <?php else: ?>
        <tr>
            <td colspan="4">No hay contratos asociados a esta consultoría jurídica.</td>
        </tr>
    <?php endif; ?>
</tbody>
