...
<section class="content">
  ...
  <section class="content">
    ...
    <tbody>
        <!-- Iterar sobre los contratos de consultoría jurídicos -->
        <?php foreach ($contratos_consultores as $contrato_consultor) : ?>
            <tr>
            <td><?= $contrato_consultor->nit_consultor ?></td> <!-- Mostrar el NIT del consultor -->
        <td><?= $contrato_consultor->nombre_consultor ?></td> <!-- Acceder al nombre completo del consultor -->
                <td>
                    <a href="<?= base_url('pdfscontratos/' . $contrato_consultor->archivo_c_j) ?>" target="_blank">
                        Abrir Archivo
                    </a>
                </td>
                <td><?= $contrato_consultor->nombre_empresa ?></td> <!-- Mostrar el nombre de la empresa del consultor -->

                <td>
                    <?php
                    // Obtener el nombre del tipo de contrato utilizando el modelo TipoContrato
                    $tipoContratoModel = new \App\Models\TipoContrato();
                    $tipoContrato = $tipoContratoModel->find($contrato_consultor->tipo_contrato_c_j);
                ...
                <td><?= $contrato_consultor->fecha_inicio_contrato_c_j ?></td> <!-- Mostrar la fecha de inicio del contrato -->
                <td><?= $contrato_consultor->fecha_finalizacion_contrato_c_j ?></td> <!-- Mostrar la fecha de finalización del contrato -->
                <td>
                    <!-- Enlace para editar el contrato consultor -->
                    <a href="<?= base_url('contrato_consultor_jj/vista_formulario_edicion_contrato_j/' . $contrato_consultor->idcontrato_c_j) ?>" class="btn btn-success" type="button">Editar contrato</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
...
<script type="text/javascript">
  ...
  <script type="text/javascript">
    ...
    $("#lista_ccontratosss_consultorj").addClass("active");
  </script>
  <?= $this->endSection(); ?>